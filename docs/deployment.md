...
注文住宅のヒアリング完了時は、氏名・電話番号を会話履歴へ残さず暗号化してD1へ保存し、`orient-chat-custom-home-leads` QueueからResend APIで通知する。Resendが発行する `send.orijyu.com` と `resend._domainkey.orijyu.com` のレコードだけを追加し、会社メールが使うルートMX・SPFは変更しない。

1. Resendでアカウントを作成し、**Domains > Add Domain** から `orijyu.com` を追加する。
...
4. Resendの **API Keys > Create API Key** で、送信権限を `orijyu.com` に限定したキーを作成する。
