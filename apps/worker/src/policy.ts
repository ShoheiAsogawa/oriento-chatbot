...
const GUIDED_SEARCH_PROMPT = /(?:購入・注文住宅・賃貸|賃貸(?:・|と|か)購入(?:・|と|か)?(?:注文住宅)?|都道府県を選んで|市区町村を選んで|家賃の上限|希望の間取りや条件|購入予算の上限|購入する物件の種類|購入物件の希望間取り|かなり手狭|登録物件情報だけでは全件を正確に絞り込めない)/u;
